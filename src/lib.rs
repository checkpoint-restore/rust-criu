...
impl Criu {
    ...
    fn recv_criu_response(fd: RawFd) -> Result<(Vec<u8>, Option<RawFd>), CriuError> {
        ...
        // CMSG_SPACE includes alignment padding and is the correct size for the buffer
        // and msg_controllen. CMSG_LEN (without padding) is only for the cmsg_len field itself.
        let cmsg_space =
        ...
        loop {
            ...
            match resp_type {
                rpc::Criu_req_type::NOTIFY => {
                    ...
                    // Only store the SCM_RIGHTS fd when the notify script is
                    ...
                    // Send notify ack (mirrors libcriu's send_notify_ack).
                    // Always send the ack so CRIU knows whether to proceed or abort.
    ...
    pub fn set_status_fd(&mut self, status_fd: i32) {
...
mod tests {
    ...
    #[test]
    fn set_status_fd_fills_criu_opts() {
        let mut criu = Criu::new().unwrap();
        criu.set_status_fd(3);

        let mut opts = rpc::Criu_opts::default();
        criu.fill_criu_opts(&mut opts);
        assert_eq!(opts.status_fd(), 3);
    }

    #[test]
    fn status_fd_default_not_set() {
        let mut criu = Criu::new().unwrap();
